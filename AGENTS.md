...
- `packages/ui/src/patterns/` — components composed on top of the
  ...
  - `ai/` — `AiButton`, `AiExplainButton`, `VoiceInputButton`,
    ...
    in-flight state (`AiButton`, three tones on top of `Button`), a lazily
    ...
    keeping the pre-transform text for a one-click undo. Unlike `Button`'s
    `swiss-primary` variant, the violet AI accent uses stock Tailwind
    colors, so it needs no tokens from the consuming app's theme.
